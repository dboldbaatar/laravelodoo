...
namespace Dboldbaatar\Odoo;

use Dboldbaatar\Odoo\Exceptions\OdooException;
...
class Odoo
{
    ...
    private function makeResponse($result, $key = null, $cast = null)
    {
        if (array_key_exists('faultCode', $result->toArray())) {
            // dd($result['faultString']);
            $faultString = json_encode($result['faultString']);
            throw new OdooException($faultString);
        }
